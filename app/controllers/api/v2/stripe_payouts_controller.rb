		...
		authorize do |user|
			begin
				...
				pullPayouts.each do |payout|
					...
					validPaymentIntents = Stripe::PaymentIntent.list({limit: 100, created: {lte: payout['metadata']['endDate'].to_time.to_i, gte: payout['metadata']['startDate'].to_time.to_i}})['data'].reject{|e| e['charges']['data'][0]['refunded'] == true}

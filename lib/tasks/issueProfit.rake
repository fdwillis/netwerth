    ...
    if !pullPayouts.blank?
      ...
      pullPayouts.each do |payoutForInvestors|
        if Date.today > DateTime.strptime(payoutForInvestors['expected_availability_date'].to_s,'%s').to_date + 1  
          principlePayoutPull = Stripe::Payout.retrieve(payoutForInvestors['metadata']['fromPayout'])
          ...
          allCurrentCardholders.each do |cardholder|
            ...
            when payoutToCardHolder
              ...
              validPaymentIntents.each do |paymentInt|
                if paymentForPayout(paymentInt['metadata']['payout'], paymentInt['metadata']['topUp'])
                  Stripe::PaymentIntent.update(paymentInt['id'], metadata: {payout: true, fromPayout: principlePayoutPull['id'], paidBy: payoutForInvestors['id']})
                end
              end
